...
// Liste des indicateurs classés par catégorie
const INDICATOR_CATEGORIES = {
    "Participation": [
        ...
        "Participants issus de zones rurales/urbaines",
    ],
    "Performance": [
...
const ProjectIndicator = () => {
    ...
    useEffect(() => {
        if (selectedProject?.indicators) {
            setSelectedIndicators(selectedProject.indicators);
        }
    }, [selectedProject]);

    // Ajout d'un indicateur standard (coché)
    ...
    // Sauvegarde des indicateurs dans Redux
    const handleSaveIndicators = async () => {
        if (!selectedProject) return;
        setIsSaving(true);

        try {
            await dispatch(updateProject({ ...selectedProject, indicators: selectedIndicators }));
    ...
    return (
        <TabPane tabId="3">
            <div className="p-4 bg-white">
                ...
                {/* Aperçu des indicateurs sélectionnés */}
