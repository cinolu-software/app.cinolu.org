...
import { handleInterview, handleEnvelope } from '@/Redux/Reducers/NotifcationSlice/notificationSlice';
import { CommonDataType} from "@/Types/Notifications/NotificationType";
import { useState} from "react";
import { Badge, Input, Label } from "reactstrap";

const PrivateNotificationContent : React.FC<CommonDataType> = ({data, ids}) => {
    const {faIcon} = useAppSelector((state)=>state.notifications);
    const [fill, setFill] = useState(false)
    const dispatch = useAppDispatch()
    const handleValue = ()=>dispatch(handleEnvelope(true))
...
export default PrivateNotificationContent;