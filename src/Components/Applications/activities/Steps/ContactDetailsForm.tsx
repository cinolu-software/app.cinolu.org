...
import {setContactDetailsFormValue} from "@/Redux/Reducers/ActivitySlice";
import ShowError from "@/Components/Applications/activities/Steps/ShowError";

const ContactDetailsForm :React.FC<ActivityFormTabContentPropsType> = ({ callbackActive }) => {
    const SelectData = ["Technology","Culture","NGO","Environment","Life cycle"]
    const {contactDetailsFormValues} = useAppSelector((state)=>state.activity)
